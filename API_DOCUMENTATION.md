...
```json
{ "phone": "9876543210", "otp": "123456", "country_code": "+91", "device_source": "web" }
```
→
```json
{
  ...
  "data": {
    ...
    "expires_in": 86400,
    "user": { "id": "uuid", "phone": "9876543210", "role": "customer", "name": null },
    "is_new_user": true
  }
}
```
...
```json
{ "token": "fcm-or-apns-or-webpush-token", "platform": "web", "device_info": { "browser": "Chrome" } }
```
...
```json
{ "data": [{ "category": {"id":"...","name":"...","slug":"..."}, "services": [...] }] }
```
...
```json
{ "name": "Ahmed", "phone": "7984561236", "email": "a@x.com", "message": "Available next week?" }
```
...
```json
{
  "name": "Hussain", "email": "hussain@example.com", "phone": "7984561235",
  "city": "Indore", "event_name": "Sundarkand Mahotsav",
  "number_of_people": 50, "preferred_date": "2026-08-15"
}
```
...
```json
{ "name": "Hussain", "email": "hussain@example.com", "phone": "7984561235", "city": "Indore", "message": "..." }
```
...
```json
{ "razorpay_order_id": "order_xxxx", "razorpay_payment_id": "pay_xxxx", "razorpay_signature": "xxxx" }
```
...
```json
{ "phone": "9876543210", "name": "Sandeep Sharma", "role": "pandit", "email": "sandeep@example.com" }
```
...
```json
{ "is_read": true, "status": "in_progress", "admin_notes": "Called back", "assigned_to": "admin-user-uuid" }
```
...
```json
{
  ...
  "pending_assignments": [ /* next 10 by respond_by */ ]
}
```
...
```json
{
  ...
  "upcoming_bookings": [ /* next 10 accepted bookings */ ],
