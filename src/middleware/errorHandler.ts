...
const collectErrorParts = (err: unknown, depth = 0): { codes: string[]; messages: string[] } => {
  const codes: string[] = [];
  const messages: string[] = [];
  if (err == null || depth > 5) return { codes, messages };

  if (typeof err === "object") {
    const e = err as { code?: unknown; message?: unknown; errors?: unknown; cause?: unknown };
    if (typeof e.code === "string") codes.push(e.code);
    if (typeof e.message === "string" && e.message) messages.push(e.message.toLowerCase());
    if (Array.isArray(e.errors)) {
      for (const inner of e.errors) {
        const nested = collectErrorParts(inner, depth + 1);
        codes.push(...nested.codes);
        messages.push(...nested.messages);
      }
    }
    if (e.cause) {
      const nested = collectErrorParts(e.cause, depth + 1);
      codes.push(...nested.codes);
      messages.push(...nested.messages);
    }
  }
...
const isDatabaseConnectionError = (err: unknown): boolean => {
  const { codes, messages } = collectErrorParts(err);
  if (
    codes.some((c) =>
      ["ECONNREFUSED", "ENOTFOUND", "ETIMEDOUT", "EAI_AGAIN", "ECONNRESET"].includes(c)
    )
  ) {
    return true;
  }
  ...
  if (isPgError(err)) {
    switch (err.code) {
      case PG_UNIQUE_VIOLATION:
        ...
        if (err.constraint === "idx_types_display_order_unique") {
          return error(
            res,
            "This display order is already used by another type. Please choose a different number.",
            409,
            "CONFLICT",
            [{ field: "display_order", message: "Each type must have a unique display order number" }]
          );
        }
