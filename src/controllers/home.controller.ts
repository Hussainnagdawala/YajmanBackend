...
export const listPopularSearchesAdmin = async (
  _req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const createPopularSearch = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    const result = await pool.query(createPopularSearchQuery, [
      label,
      link_url ?? null,
      display_order,
      row_number,
...
export const updatePopularSearch = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    const result = await pool.query(updatePopularSearchQuery(fields), [
      req.params.id,
      ...values,
    ]);
    if (!result.rows[0])
      throw new AppError("NOT_FOUND", "Popular search not found", 404);
...
export const deletePopularSearch = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const deletePopularSearchPermanently = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const listBanners = async (
  ...
  try {
    ...
    const result = await pool.query(listActiveBannersByPosition, [type]);
    return success(res, result.rows);
  } catch (err) {
    next(err);
  }
...
export const updateBanner = async (
  ...
  try {
    ...
    const result = await pool.query(updateBannerQuery(fields), [
      req.params.id,
      ...values,
    ]);
    if (!result.rows[0])
...
export const deleteBannerPermanently = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (result.rows[0].mobile_image_url)
      await deleteFromS3(result.rows[0].mobile_image_url);
...
export const updateTestimonial = async (
  ...
  try {
    ...
    const result = await pool.query(updateTestimonialQuery(fields), [
      req.params.id,
      ...values,
    ]);
    if (!result.rows[0])
      throw new AppError("NOT_FOUND", "Testimonial not found", 404);
...
export const deleteTestimonialPermanently = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
